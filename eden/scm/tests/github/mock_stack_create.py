...
# Mock for the first `sl pr submit` of a two-commit stack with
# github.pr-workflow=stack: PRs are created with chained bases and then
# linked into a native GitHub stack. Note that expect_update_pr_request uses
# base=None: the stack workflow must never include baseRefName in
# updatePullRequest (GitHub rejects the whole mutation for stacked PRs), so
# a request carrying baseRefName fails with MockRequestNotFound here.
