...
# on GitHub has diverged from the local stack (here: wrong order), but every
# member belongs to the local stack: submit automatically dissolves the stack
# and re-links it to match.
...
def setup_mock_github_server(ui) -> MockGitHubServer:
    ...
    github_server.expect_get_stack_request(42).and_respond(
        [stack_json(44, [43, 42, 45])]
    )
    github_server.expect_unstack_request(44, [43, 42, 45]).and_respond({})
